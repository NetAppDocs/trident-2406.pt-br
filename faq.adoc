...
Sim, Astra Trident pode ser instalado offline. Consulte link:../trident-install/kubernetes-deploy.html["Saiba mais sobre a instalação do Astra Trident"^].
...
Você pode gerar e modificar localmente os arquivos de manifesto exatos que o Astra Trident usa para instalação, se necessário. Consulte link:trident-install/kubernetes-customize-deploy-tridentctl.html["Personalize a instalação do tridentctl"^].
