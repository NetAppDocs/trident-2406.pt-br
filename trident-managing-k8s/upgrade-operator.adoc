...
.Passos
. Se você link:../trident-install/kubernetes-deploy-helm.html#deploy-the-trident-operator-and-install-astra-trident-using-helm["Instalado Astra Trident usando o Helm"], pode usar  `helm upgrade trident netapp-trident/trident-operator --version 100.2406.0` para atualizar em uma única etapa. Se você não adicionou o repositório Helm ou não consegue usá-lo para atualizar:
+
