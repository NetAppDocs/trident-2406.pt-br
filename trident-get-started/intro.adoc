...
Para experimentar, solicite acesso ao recurso "Implantação e Clonagem Fáceis de Armazenamento Persistente para Cargas de Trabalho em Contêineres" link:https://www.netapp.com/test-drives/["Unidade de teste do NetApp"^] usando uma imagem de laboratório pronta para uso. O teste drive oferece um ambiente de sandbox com um cluster Kubernetes de três nós e o Astra Trident instalado e configurado. Esta é uma ótima maneira de você se familiarizar com o Astra Trident e explorar seus recursos.
